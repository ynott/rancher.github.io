...
title: Adding Load Balancers layout: rancher-default-v1.2 version: v1.2 lang: en redirect_from: - /rancher/rancher-ui/applications/stacks/adding-balancers/ - /rancher/latest/en/cattle/adding-load-balancers/
...
For example, if you were load balancing a web server and wanted both `http` and `https` being served on ports 80 and 443 respectively, you would do the following:
  ...
  3. In the *Listening Ports* section of the screen, create an entry to map the incoming port 80, with the `http` protocol and SSL *unchecked*.
  4. And then add another port mapping the incoming port 443, `http` protocol and SSL *checked*.
