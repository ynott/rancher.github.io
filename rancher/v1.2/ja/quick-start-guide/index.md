...
** Stacks **ページに移動します。まだサービスがない場合は、ウェルカム画面の** Define a service **ボタンをクリックします。 Rancherに既にサービスが設定されている場合は、既存のスタックの**サービスの追加**をクリックするか、新しいスタックを作成してサービスを追加できます。 新しいスタックを作成する必要がある場合は、**スタックを追加**をクリックし、名前と説明を入力して**作成**をクリックします。 次に、**サービスの追加**をクリックします。
